...
Dataset : `power_consumption.csv`

| Column | Description |
| --- | --- |
| `Date` | Tanggal pencatatan (berformat `YYYY-MM-DD`) |
| `Power_Usage` |  Konsumsi listrik harian (dalam `Megawatt Hour (MWh)`) |
...
Anda bekerja sebagai Data Scientist di sebuah pabrik manufaktur besar. Pabrik ini ingin mengoptimalkan penggunaan listrik mereka dan membutuhkan prediksi konsumsi listrik untuk perencanaan jangka pendek. Anda diminta menganalisis data konsumsi listrik harian pabrik dan memprediksi penggunaan listrik untuk 30 hari ke depan. Hasil analisis ini akan digunakan untuk perencanaan maintenance, optimasi jadwal produksi, dan negosiasi kontrak dengan penyedia listrik.
...
5. Ketentuan Exploratory Data Analysis:
   ...
   2. Diperlukan untuk melakukan Time Series Decomposition dengan model `additive` atau `multiplicative` sesuai dengan pilihan dan justifikasi yang tepat. Proses ini diperlukan untuk mendapatkan pola seasonal data dan periode seasonal untuk parameter `S` di model SARIMA.
...
7. Pada bagian Model Inference :
   - Lakukanlah prediksi selama 30 hari ke depan dari tanggal terakhir dari dataset yang diberikan.
   - Gunakan model terbaik berdasarkan hasil analisis yang Anda lakukan pada bagian Model Evaluation.
